...
Version 2:
- data streaming via Python iterators
- no intermediate files
- sequential batch processing
- fastq parsing via pysam
- no parallelization
...
def batch_iterator(iterator: Iterator, batch_size: int) -> Iterator[List]:
    """
    Group items from iterator into batches.
...
def reporting_thresholds() -> Iterator[int]:
    """
    Report progress every 100K reads until 1M, then every 1M reads thereafter.
    
    Yields:
        Read counts at which to log progress
    """
    threshold = 100_000
...
def process_batch(batch: List[ReadPair], patterns_r1: List[str], patterns_r2: List[str]) -> Tuple[List[ReadPair], Dict[str, int]]:
    """
    Process a batch of read pairs - reorient (swap R1/R2) based on pattern matching.
    
    Args:
        batch: List of ReadPair objects
        patterns_r1: R1 search patterns
        patterns_r2: R2 search patterns
...
def main(sequences_r1, sequences_r2='', in_fastq_r1=None, in_fastq_r2=None,
         ...
         cpus=None, batch_size=5000, nreads=None, force_gzip=False, log_file=None,
    ...
    logging.info("="*60)
    logging.info("FASTQ Read Reorientation v2")
    logging.info("="*60)
    ...
    logging.info(f"Configuration: batch_size={batch_size}")
    ...
    for batch in batches:
        results, stats = process_batch(batch, patterns_r1, patterns_r2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Reorient paired-end FASTQ reads based on search patterns.',
